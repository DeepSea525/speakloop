import type { ChatMessage, CoachingResult, ExtractionResult, PracticeFeedback } from "@/lib/types";
      ...
      content: [
        {
          type: "input_text",
          text:
            "你是一个轻量英语表达陪练节点。用户可能输入中文、中英混合或不完整英文。请理解真实意图，给出短、自然、能立刻说出口的英文。只返回合法 JSON，不要 markdown。",
        },
      ],
    },
    {
...
export function parseCoachingJson(text: string): CoachingResult {
  const trimmed = text.trim().replace(/^```json\s*/i, "").replace(/^```\s*/i, "").replace(/```$/i, "").trim();
  const parsed = JSON.parse(trimmed) as Partial<CoachingResult>;
...
function sceneFallback(value: unknown) {
